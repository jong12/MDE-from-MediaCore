...
from monodepth_model_resnet import *
...
#parser.add_argument('--val_filenames_file',        type=str,   help='path to the validation filenames text file', required=True)
...
#############################################################
        ...
        train_saver.save(sess, args.log_directory + '/' + args.model_name + '/model', global_step=start_step+num_total_steps)
...
def test(params):
    ...
    processing_time = np.zeros(shape=(num_test_samples), dtype=np.float32)
    ...
    for step in range(num_test_samples):
        ...
        processing_time[step] = time.clock() - tic
        print('done in {} seconds!'.format(time.clock() - tic))
    print("average processing time",np.mean(processing_time))
    print('done.')
    ...
    for i in range(num_test_samples):
        ...
        path = os.path.join(args.test_out_file + str(i).zfill(4)+'.jpg')
        imsave(path, image)
