...
class MonodepthModel(object):
    ...
    def resconv(self, x, num_layers, stride,scope):
        do_proj = tf.shape(x)[3] != num_layers or stride == 2
        shortcut = []
        conv1 = self.conv(x,         num_layers, 1, 1,scope+'r1')
        conv2 = self.conv(conv1,     num_layers, 3, stride,scope+'r2')
        conv3 = self.conv(conv2, 4 * num_layers, 1, 1,scope+'r3', activation_fn=None)
        if do_proj:
            shortcut = self.conv(x, 4 * num_layers, 1, stride, scope+'sc', activation_fn=None)
        else:
            shortcut = x
        return tf.nn.elu(conv3 + shortcut)

    def resblock(self, x, num_layers, num_blocks,scope):
        out = x
        for i in range(num_blocks - 1):
            out = self.resconv(out, num_layers, 1,scope)
        out = self.resconv(out, num_layers, 2,scope)
        return out
    ...
    def build_vgg(self):
        ...
        with tf.variable_scope('decoder'):
            ...
            upconv2 = upconv(iconv3,  32, 3, 2,scope='upconv2') #H/2
    ...
    def build_resnet50(self):
        ...
        with tf.variable_scope('encoder'):
            conv1 = conv(self.model_input, 64, 7, 2,scope='conv1') # H/2  -   64D
            pool1 = self.maxpool(conv1,           3) # H/4  -   64D
            conv2 = self.resblock(pool1,      64, 3,scope='conv2') # H/8  -  256D
            conv3 = self.resblock(conv2,     128, 4,scope='conv3') # H/16 -  512D
            conv4 = self.resblock(conv3,     256, 6,scope='conv4') # H/32 - 1024D
            conv5 = self.resblock(conv4,     512, 3,scope='conv5') # H/64 - 2048D
        ...
        # DECODING
        with tf.variable_scope('decoder'):
            upconv6 = upconv(conv5,   512, 3, 2,scope='upconv6') #H/32
            concat6 = tf.concat([upconv6, skip5], 3)
            iconv6  = conv(concat6,   512, 3, 1,scope='iconv6')
