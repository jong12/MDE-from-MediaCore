...
class MonodepthDataloader(object):
    ...
    def __init__(self, data_path, filenames_file, params, dataset, mode):
        ...
        self.gradient_gl_batch = None
        self.gradient_gr_batch = None
        ...
        if mode == 'test' and not self.params.do_stereo:
            ...
            gradient_gl_path = tf.string_join([self.data_path, split_line[2]])
            gradient_gr_path = tf.string_join([self.data_path, split_line[3]])
            print('gradient_gr_path',gradient_gr_path)
            
            left_image_o  = self.read_image(left_image_path)
            right_image_o = self.read_image(right_image_path)

            gradient_gl_o = self.read_image(gradient_gl_path)
            gradient_gr_o = self.read_image(gradient_gr_path)
        ...
        if mode == 'train':
            ...
            gradient_gl = tf.cond(do_flip > 0.5, lambda: tf.image.flip_left_right(gradient_gr_o),  lambda: gradient_gl_o)
            gradient_gr = tf.cond(do_flip > 0.5, lambda: tf.image.flip_left_right(gradient_gl_o),  lambda: gradient_gr_o)
            ...
            gradient_gl.set_shape([None, None, 3])
            gradient_gr.set_shape([None, None, 3])
            ...
            self.left_image_batch, self.right_image_batch, self.gradient_gl_batch, self.gradient_gr_batch = tf.train.shuffle_batch([left_image, right_image, gradient_gl, gradient_gr], params.batch_size, capacity, min_after_dequeue, params.num_threads)
