...
from monodepth_model_gradient_4layer import *
from monodepth_dataloader_gradient import *
...
def train(params):
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        gradient_gl = dataloader.gradient_gl_batch
        gradient_gr = dataloader.gradient_gr_batch
        ...
        gradient_gl_splits = tf.split(gradient_gl, args.num_gpus, 0)
        gradient_gr_splits = tf.split(gradient_gr, args.num_gpus, 0)
        tower_grads  = []
        tower_losses = []
        tower_losses_edge = []
        ...
        with tf.variable_scope(tf.get_variable_scope()):
            for i in range(args.num_gpus):
                with tf.device('/gpu:%d' % i):
                    ...
                    model = MonodepthModel(params, args.mode, left_splits[i], right_splits[i], gradient_gl_splits[i], gradient_gr_splits[i], reuse_variables, i)

                    loss = model.total_loss
                    
                    edge_loss = model.loss_edge

                    tower_losses_edge.append(edge_loss)
        ...
        total_loss_edge = tf.reduce_mean(tower_losses_edge)
        ...
        for step in range(start_step, start_step+num_total_steps):
            ...
            _, loss_value, loss_edge_value = sess.run([apply_gradient_op, total_loss, total_loss_edge])
            duration = time.time() - before_op_time
            if step and step % 100 == 0:
                examples_per_sec = params.batch_size / duration
                time_sofar = (time.time() - start_time) / 3600
                training_time_left = (num_total_steps / (step-start_step) - 1.0) * time_sofar
                print_string = 'batch {:>6} | examples/s: {:4.2f} | loss: {:.5f} | edge_loss: {:.5f} | time elapsed: {:.2f}h | time left: {:.2f}h'
                print(print_string.format(step, examples_per_sec, loss_value, loss_edge_value, time_sofar, training_time_left))
...
def test(params):
    ...
    model = MonodepthModel(params, args.mode, left, right, gradient_gl, gradient_gr)
    ...
    for i in range(num_test_samples):
        ...
        path = os.path.join(args.test_out_file + str(i).zfill(5)+'d.jpg')
        imsave(path, image)
