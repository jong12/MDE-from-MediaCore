...
class MonodepthModel(object):
    ...
    def __init__(self, params, mode, left, right, gradient_gl, gradient_gr,reuse_variables=None, model_index=0):
        self.params = params
        self.mode = mode
        self.left = left
        self.right = right
        self.gradient_gl = gradient_gl
        self.gradient_gr = gradient_gr 
    ...
    def get_disp(self, x, scope):
        disp = 0.55 * self.conv(x, 2, 3, 1, scope=scope,activation_fn=tf.nn.sigmoid)
        return disp
    ...
    def build_vgg(self):
        ...
        with tf.variable_scope('encoder'):
            ...
            conv4 = self.conv_block(conv3,            256, 3,scope='conv4') # H/16
            conv5 = self.conv_block(conv4,            512, 3,scope='conv5') # H/32
        ...
        with tf.variable_scope('decoder'):
            ...
            upconv4 = upconv(iconv5, 128, 3, 2,scope='upconv4') #H/8 (64*32)
...
#est_depth_edge=edge_layer4
            #self.edge_layer4 = conv(conv_n4, 2, 1, 1, scope='edge_layer4',activation_fn=tf.nn.sigmoid)
...
#######################################################################################################################################
    ...
    def build_outputs(self):
        ...
        with tf.variable_scope('edge'):
            self.edge_est  = [self.edge_layer1, self.edge_layer2, self.edge_layer3, self.edge_layer4]
            self.edge_est_l = [tf.expand_dims(e[:,:,:,0], 3) for e in self.edge_est]

            self.edge_est_r = [tf.expand_dims(e[:,:,:,1], 3) for e in self.edge_est]
    ...
    def build_losses(self):
        with tf.variable_scope('losses', reuse=self.reuse_variables):
            ...
            #self.l1_left = [tf.multiply(tf.abs( self.left_est[i] - self.left_pyramid[i]),((self.label_pyramid[i]/255)+1)) for i in range(4)]
...
#######################################################################################################################################
           # Edge
#            self.gl_resize = self.downsample_nn(self.gradient_gl, 4)
#            self.gr_resize = self.downsample_nn(self.gradient_gr, 4)
#            
#            self.sobelgl = 0.5*tf.image.sobel_edges(self.gl_resize)[:,:,:,:,0]+0.5*tf.image.sobel_edges(self.gl_resize)[:,:,:,:,1]
#            self.sobelgr = 0.5*tf.image.sobel_edges(self.gr_resize)[:,:,:,:,0]+0.5*tf.image.sobel_edges(self.gr_resize)[:,:,:,:,1]

#            self.sobelestl = 0.5*tf.image.sobel_edges(self.est_depth_edgel)[:,:,:,:,0]+0.5*tf.image.sobel_edges(self.est_depth_edgel)[:,:,:,:,1]
#            self.sobelestr = 0.5*tf.image.sobel_edges(self.est_depth_edger)[:,:,:,:,0]+0.5*tf.image.sobel_edges(self.est_depth_edger)[:,:,:,:,1]
            
#            self.loss_l = [tf.abs(self.sobelestl-self.sobelgl)]
#            self.loss_r = [tf.abs(self.sobelestr-self.sobelgr)]
#            self.loss_edge = tf.add_n(self.loss_l + self.loss_r)

           # Edge
            self.gl_resize = self.downsample_nn(self.gradient_gl, 4)
            self.gr_resize = self.downsample_nn(self.gradient_gr, 4)

            self.gt_l = tf.expand_dims(self.gl_resize[:,:,:,0], -1)
            self.gt_r = tf.expand_dims(self.gr_resize[:,:,:,0], -1)

            
            self.loss_l = [tf.abs(self.est_depth_edgel - self.gt_l)]
            self.loss_r = [tf.abs(self.est_depth_edger - self.gt_r)]
            self.loss_edge = tf.add_n(self.loss_l + self.loss_r)


#######################################################################################################################################
            # TOTAL LOSS
            self.total_loss = self.image_loss + self.params.disp_gradient_loss_weight * self.disp_gradient_loss + self.params.lr_loss_weight * self.lr_loss + 1*self.loss_edge
    ...
    def build_summaries(self):
        ...
        with tf.device('/cpu:0'):
            ...
            for i in range(4):
                ...
                tf.summary.image('edge_est_l' + str(i), self.edge_est_l[i], max_outputs=4, collections=self.model_collection)
                tf.summary.image('edge_est_r' + str(i), self.edge_est_r[i], max_outputs=4, collections=self.model_collection)
