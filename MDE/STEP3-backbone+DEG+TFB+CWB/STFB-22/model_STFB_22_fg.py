...
class MonodepthModel(object):
    ...
    def nonlocal_block(self, x, c, scope): #c=channel*1/2
        ...
        first = tf.case({tf.equal(self.flag, tf.constant(0)):f0, tf.equal(self.flag, tf.constant(1)):f1},default=f5)
        ...
        if scope =='nonlocal_3to4':
            ...
            if first == 0:
                nonlocal1_concat = tf.concat([nonlocal1, nonlocal1], 2, name=scope+'_concatpre0') #concat [B,H,2W,2c]
            else:
                nonlocal1_concat = tf.concat([nonlocal1_t_1, nonlocal1], 2, name=scope+'_concatpre1') #concat [B,H,2W,2c]

            nonlocal_conv2 = slim.conv2d(nonlocal1_concat, c, 1, 1, 'VALID', scope=scope+'_conv2') #1*1conv [B,H,2W,c]
            print('***********************nonlocal_conv2',nonlocal_conv2.shape)
            nonlocal_conv3 = slim.conv2d(nonlocal1_concat, c, 1, 1, 'VALID', scope=scope+'_conv3') #1*1conv [B,H,2W,c]
        ...
        print('***********************nonlocal_conv',nonlocal_conv.shape)
